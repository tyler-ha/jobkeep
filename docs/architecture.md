...
| **docker-compose** | Replaces the manual `docker run` in the README. One file, and Docker is on every ad. | **P4** — `run.cmd` (Phase 6.2) now does what this was for, so the gap is mostly closed by other means |
