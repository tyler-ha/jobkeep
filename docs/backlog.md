...
### Added by Phase 9 gap 1 (2026-09-04)

| Candidate | What it is | Cost / size | Likely home | Notes |
|---|---|---|---|---|
| **Today says "never checked against a CV"** | A block on the Today screen naming the applications with no stored match check | Low — the data is already on the wire | **P3, with the Phase 6 visual pass** | Phase 9 named this as gap 1's *second* caller, and the second caller is what justified building the contract method. It did not ship with it: `ApplicationListItem.match` is null for exactly these rows, so the data is there, but Today has no dropped affordance waiting for it the way Applications did, and its current copy is not telling a lie — it simply does not mention match checks. So this is a **screen design question** (what does the block say, where does it sit, how many rows does it name) rather than a backend one, which puts it with the visual pass rather than in front of it. |
