...
*Regenerated 2026-09-04, in a fresh session, after Phase 6.5 finished in full. The
...
- **Fresh in + Output** — closer to "how much thinking and writing happened".
  Across the whole project that is **40.5M against a 1390.6M gross — 2.9%**.
  The ratio was ~4% when the gross was 302M and has drifted DOWN as the project
  grew, which is the cost rule restated: replay scales with how much context
...
| **Phase 7** — data integrity + natural key | 2 | 343 | 52.9M | `IAuditable` + `AuditSaveChangesInterceptor` (F8), `xmin` (F7), DB-side defaults as a convention loop (F11), two CHECK constraints (F12), eleven bounded columns (F13), the two parked indexes (F14), and the case-insensitive natural key across all three tables — one migration, with a hand-written duplicate-merge step ahead of the new unique indexes. Suite 228 → 239. **Now final, and it is the LARGEST CORRECTION IN THE LEDGER: logged from inside itself at 100+ turns / 12.9M, finished at 343 / 52.9M — 4.1x**, past Phase 4.5's review session (3.9x) and Phase 5's wrap-up (3.5x). Two things make it the worst case rather than just the newest. It was logged *early* in the first of **two** sessions, so the row missed both the rest of that session and the whole of a second one (219 turns / 31.3M) — every previous correction was one session growing, this is a phase that was not finished when it was logged. And nothing in the row said so: it claimed "provisional" for the usual reason and was wrong by a different, larger one. **The lesson is narrower than "log later": do not log a phase whose next session is already implied.** Two things worth the row otherwise. First, **the phase found its own scope**: the plan named the find-or-create lookups and missed two batch resolvers and an in-batch `StringComparer.Ordinal`, all of which the new index would have turned from silent duplicates into failed inserts — adding a constraint makes every existing near-miss loud, which is the point and also the blast radius. Second, **the defect tests paid off exactly as designed**: two of them broke on the migration and announced the fix, and one of the phase's own new tests was wrong in a way that taught a rule (`UpdatedAtUtc` is about a row, not its children). |
| **Phase 13** — module-owned Clean Architecture | 12 | 1832 | 316.6M | The largest phase in the project by a wide margin — **more than Phase 4.5's 180.4M and Phase 2.2's 62.2M put together**, and roughly a quarter of everything the project has cost. Six sub-steps, each ending runnable: 13.1 ten projects, 13.2 (a–e) the logical boundary and four contracts, 13.3 (a–c) five Postgres schemas and six cross-schema foreign keys dropped then replaced in application code, 13.4 the mediator, 13.5 five controllers, 13.6 namespaces and the decision record. Suite 239 → 285. **Now final: 1811+ / 311.8M+ when logged, 1832 / 316.6M when the last session stopped — 1.02x, the smallest ratio in the ledger by a distance** (past Phase 14's 1.06x and Phase 6.1's 1.10x), because it was logged with barely twenty turns left to run. Read it beside Phase 7 in this same table: same "provisional" wording, 1.02x here and 4.1x there. The word never carried the size of the gap, and now two rows one apart prove it. Attributed by branch (`phase-13/module-boundaries` 9 sessions / 248.3M, `phase-13/dispatch` 1 / 21.4M, `phase-13/controllers` 2 / 46.9M); planning that happened on `develop` is **not** counted here, so the true figure is higher again. Three things the row is for. First, **the number is the argument for splitting incrementally, not against it**: no single session exceeded 65.4M, where Phase 4.5's review alone hit 100.2M, because each sub-step ended and the next one started cold. The phase is expensive in total and cheap per session, which is exactly the shape `CLAUDE.md`'s cost rule predicts. Second, **the plan was rewritten before any code moved** — a layer-first migration was drafted and discarded the same day on one sentence from the user, and that reversal cost one session instead of the four it would have cost after the projects existed. Third, **it kept finding defects in things that already passed**: a `DispatchTests` line that had been loading the wrong assembly and checking none of the Skills module, six dropped foreign keys where the plan said five, and a `No_module_takes_a_context` test that would have gone vacuous the moment `AppDbContext` was deleted. A refactor that touches everything is also the cheapest audit available. |
| **Phase 14** — skill vocabulary | 1 | 348 | 77.3M | `SkillKind` in Contracts, `skills.skill_aliases` resolved inside `SkillCatalog` (no call site changed), a 228-skill / 322-alias embedded seed, and the extraction rewritten to ask for soft skills. Suite 268 → 281. **Now final, and the fifteenth consecutive upward correction:** logged from inside itself at 335 turns / 72.6M, finished at **348 / 77.3M**. At 1.06x it is the smallest ratio in the ledger by a distance, past Phase 6.1's 1.10x, and for the same mundane reason — it was logged thirteen turns from the end. The size of a correction measures how much session was left, not how wrong the estimate was. The session also carried the Phase 6 visual re-walk (a separate commit), which is part of why the turn count is high for the size of the diff. Three things worth the row. First, **the phase was diagnosed from the database, not from the code**: `SELECT "Name","Category" FROM skills.skills` showed `Agile`/`Agile Methodologies` and `Docker`/`containers` side by side, which made a vague complaint ("it didn't recognise all the skills") into two specific causes in one query. Second, **the fix cost almost nothing at the call sites** — five find-or-create callers got aliasing without being touched, because 13.2c had already put the natural key in one place. Paying for that boundary once bought this phase cheaply. Third, **the end-to-end run against the real model changed the design mid-phase**: it returned `"Excellent communication skills"` rather than `Communication`, and the answer was one line of `[Description]` rather than the fifty aliases the seed file was drifting towards. Verifying against the actual model, not just the tests, is what caught it. |
| **Phase 6.5 group 6** — the upload stops blocking | 1 | 348 | 71.1M | `POST /imports` stopped calling the model: it extracts, saves and returns with the row in a new `ImportStatus.Parsing`, and `ImportParseWorker` structures it in the background. No migration — `document_imports.Status` had no CHECK constraint, so the queue is a column that already existed. Suite 285 → 290. **Now final, and the seventeenth consecutive upward correction:** logged from inside itself at 320 turns / 62.1M, finished at **348 / 71.1M**. At 1.14x it is among the smallest ratios in the ledger, for the reason two earlier rows already established — it was written 28 turns from the end, so the size of the correction measures how much session was left, not how wrong the row was. Three things worth the row. First, **the design was built, shipped and reversed in one session**: the first version had the client drive the model from the review screen, and one sentence from the user — *"not blocking then why don't you use queue to make it happen?"* — replaced it with the worker. The reversal cost roughly a third of the session and would have cost a phase if it had shipped and been discovered later. Second, **the plan was wrong about the code in three places and reading beat trusting it**: `/reparse` did not accept a `Parsing` row, never set `Status`, and swallowed model failures in a way that would have stranded every failed parse. Third, **the end-to-end run found a bug no test could**: `docker compose up --build` had been broken on `develop` since the match-check rename, because `src/Dockerfile` lists the projects a second time by hand and the rename swept only `src/**/*.cs`. CI reads the `.slnx`, which was correct. The stack has to be *run*, not built. |
| **Phase 6.5 group 4** — paste the ad | 2 | 127 | 16.8M | `POST /imports/text` and the `importText` mutation take a pasted advertisement instead of a file. Suite 290 → 299, `web` 49 → 50, no migration. **Final** — one build session (`5b416a79`, 119 / 16.1M) plus the eight turns of `effbf034` that merged it. The cheapest feature row since Phase 6.2, and the reason is worth naming because it is a decision, not luck: **the handler delegates to `ImportDocument` through the mediator instead of calling `IDocumentTextExtractor` itself, which is what the phase plan specified.** Doing less than the plan asked is what makes "a paste and an uploaded `.txt` are the same import" a property of the call graph rather than four behaviours that agree by inspection — same content hash, same extracted text, same format, same status, asserted as identity by `ImportTextTests`. The plan's version would have recreated the hash, the filename truncation, the save-before-parse ordering and the enqueue as a second copy. **A plan written in an earlier session is cheap to run and is still worth disobeying.** |
| **The `Documents` config section** | 1 slice | 73 | 8.6M | `appsettings.json` gained the five policy limits that `DocumentsModule.cs` had been binding out of a section existing in no file. **A no-op at runtime — every value equals the C# default it replaced** — and that is the row rather than a caveat on it. Final; it is a 73-turn slice of `effbf034`, not a session of its own. Two things it bought. First, **it reverses the direction of authority**: appsettings now wins over the default, so a dropped zero in `MaxBytes` silently gives a 512 KB cap where that was previously impossible. `DocumentOptionsConfigurationTests` exists for exactly that, was verified non-vacuous by perturbing a value until it failed, and its doc comment states the hole it does *not* cover — a misspelled key falls back to the same default and passes. Second, **it drew the line the section does not cross**: `DraftLimits` and `CommitImport`'s clip literals stay constants because they are column widths, not policy — a setting that disagrees with `varchar(20000)` does not widen the column, it moves the failure from a clip we control to an `INSERT` that 500s. `ParseInBackground` stays out for the same shape of reason: a test seam declared in a config file is an invitation to turn it off, which would strand every import in `Parsing` with no exit. **Config is for the values a deployment may legitimately disagree with, and that is a smaller set than "the constants".** |
| **Phase 8** — soft delete | 1 running | 240+ | 43M+ | `ISoftDeletable` converted centrally in `AuditSaveChangesInterceptor` so `Remove()` means archive, `HasQueryFilter` on the three entities with a delete slice, three restore slices on both surfaces, `?includeArchived`, a filtered unique index on `resumes.LabelNormalized`, and the three published views re-cut by hand. Two migrations, suite 303 → 314, web 50 → 52. **Provisional, and doubly so.** It is logged from inside the session it measures — seventeen consecutive rows have been corrected upward and there is no reason this is the exception — and the session's own row is hard to attribute: it opened on `docs/roadmap-aws-dropped` (the branch the ledger labels it by) and only moved to `phase-8/soft-delete` at the commit, so the branch column will not find it where a reader looks. The session table is **not** refreshed here on purpose; that belongs with the already-scheduled Phases 8-14 backfill, which the ledger's own rule says to do in a fresh session, and refreshing it from inside a running session would just add an eighteenth number to correct. Three things worth the row. First, **the plan's cost estimate was wrong by an order of magnitude in the direction nobody checks**: it called this the highest front-end blast radius on the roadmap and costed five list routes; one screen changed, because the delete affordances it assumed would be converted did not exist — `deleteApplication` was exported from `api.ts` and called by nothing. An estimate written against a *planned* UI decays as fast as the plan does, and this one was written before Phase 6 shipped and never re-checked against the eight screens that actually got built. Second, **the expensive part was invisible from the plan**: three raw-SQL views that `HasQueryFilter` cannot reach, and the 13.3c delete notifications, which had to *stop* being published because an archive must not destroy a stored model result about a row that survived. Both are reasoning, not typing, and neither appears anywhere in the phase doc. Third, **seven existing tests had to be flipped rather than written** — the suite grew by eleven but changed in twenty-odd places, so test *count* understated the test work by roughly half. |
| **The match-check rename** — ATS check → match check | 2 | 237 | 24.0M | `Jobkeep.Modules.Match`, `match_results`, `POST`/`GET /applications/{id}/match-check`, `runMatchCheck`, `MatchCheck.tsx`. One migration, suite 283 → 285. **Final.** Cheap for a rename touching every layer, and the reason is the one Phase 5 and 6.2 both found: it ran off a plan written earlier and blocked on 13.5, so no deliberation replayed. Two findings. First, **the Postgres schema stayed `ats` deliberately** — each module's `__EFMigrationsHistory` lives in its own schema and EF resolves it *before* applying anything, so renaming the schema would have made EF conclude nothing had been applied and re-run `InitialCreate` against live tables. Second, **a bulk rename script is a blunt instrument**: it rewrote a shipped migration file (reverted by hand — a rewritten migration history is silent breakage on any database with rows), it walked into a stale git worktree, and it missed `src/Dockerfile` entirely. The miss was not found for a day, by which time the rename was on `develop`. |
...
| | | | | |
| **Total** | **70** | **9077** | **1426.4M** | Regenerated 2026-09-04, second pass that day, in a fresh session as the rule below requires. **The three sessions that closed Phase 6.5 are now logged** — group 4's build, the session that shipped it and added the `Documents` config section, and group 6's correction from 62.1M to 71.1M. **The project has passed 1.43 billion tokens across 70 sessions**, against 1.39 billion earlier the same day; the 36M difference is one feature, one config section and one correction. Everything below this sentence was true at the previous regeneration and still is. **The stale-table admission that stood here is discharged**: the session table below is fresh, and the gaps it named are closed — Phase 7 and Phase 13 are final, and the match-check rename and Phase 6.5 group 6 have rows. **Phases 8-12 have no rows because they do not exist yet**; the previous note called them missing, which was wrong. They are unbuilt (soft delete, the backend gaps, the parked AWS deploy, auth, feature expansion), and the renumber is what made a gap in the numbers look like a gap in the ledger. **What is still not attributed: three `develop` sessions since 2026-09-01 (572 turns / 95.8M)** covering Phase 6.5 groups 1-3 and 5, Phase 6.6, the roadmap reorder and cross-phase planning. The script attributes by branch and all three ran on `develop`, so no honest split exists without reading transcripts; the "Roadmap reorder" row below is the provisional slice of it and stays that way. They are in this total, so the total is right and only the shape is coarse. **The project has passed 1.39 billion tokens across 67 sessions**, against 1.25 billion when this was last rebuilt a day ago — Phase 13's tail, Phase 14, the rename and group 6 are that 137M. Two corrections landed with this regeneration and they disagree instructively: Phase 13 at **1.02x** and Phase 7 at **4.1x**, the smallest and largest ratios in the ledger, one row apart, both previously marked "provisional" in the same words. |
...
| 2026-08-29 13:28 | `phase-6.2/frontend-scaffold` | 36 | 245k | 2.4M | 25k | **2.6M** | `879b96a1` |
...
| 2026-08-31 03:23 | `phase-6.2/frontend-scaffold` | 150 | 242k | 17.3M | 115k | **17.7M** | `ec554da1` |
| 2026-09-01 02:02 | `develop` | 228 | 666k | 43.9M | 292k | **44.9M** | `0e202588` |
| 2026-09-01 03:43 | `develop` | 153 | 494k | 21.9M | 227k | **22.6M** | `2671ea66` |
| 2026-09-01 04:41 | `phase-7/data-integrity` | 124 | 457k | 21.0M | 140k | **21.6M** | `8ff22e3c` |
| 2026-09-01 05:12 | `phase-7/data-integrity` | 219 | 351k | 30.8M | 171k | **31.3M** | `3db12cfb` |
| 2026-09-01 06:44 | `phase-13/module-boundaries` | 182 | 564k | 30.6M | 278k | **31.4M** | `20f7186f` |
| 2026-09-01 07:21 | `phase-13/module-boundaries` | 149 | 440k | 23.4M | 142k | **24.0M** | `5048159f` |
| 2026-09-01 10:55 | `phase-13/module-boundaries` | 196 | 465k | 35.6M | 223k | **36.3M** | `c9d52230` |
| 2026-09-01 11:47 | `phase-13/module-boundaries` | 209 | 433k | 36.9M | 244k | **37.6M** | `43328081` |
| 2026-09-01 13:05 | `phase-13/module-boundaries` | 62 | 650k | 6.8M | 53k | **7.5M** | `91e40928` |
| 2026-09-01 13:05 | `phase-13/module-boundaries` | 56 | 268k | 6.2M | 43k | **6.5M** | `efcb19a1` |
| 2026-09-02 00:12 | `phase-13/module-boundaries` | 285 | 631k | 64.5M | 232k | **65.4M** | `4aa7a2dc` |
| 2026-09-02 03:05 | `phase-13/module-boundaries` | 191 | 487k | 32.5M | 222k | **33.2M** | `2b7cdf9f` |
| 2026-09-02 13:18 | `phase-13/module-boundaries` | 64 | 543k | 5.8M | 58k | **6.4M** | `89564ca1` |
| 2026-09-03 04:12 | `phase-13/dispatch` | 135 | 429k | 20.8M | 120k | **21.4M** | `fb54ca63` |
| 2026-09-03 04:45 | `develop` | 191 | 492k | 27.6M | 168k | **28.3M** | `9e93c0a2` |
| 2026-09-03 05:33 | `phase-14/skill-vocabulary` | 348 | 854k | 76.2M | 295k | **77.3M** | `488cb7e0` |
| 2026-09-03 06:47 | `phase-13/controllers` | 146 | 452k | 22.4M | 177k | **23.1M** | `32cb4a9b` |
| 2026-09-03 07:25 | `phase-13/controllers` | 157 | 430k | 23.2M | 177k | **23.8M** | `ec4aed4f` |
| 2026-09-03 08:06 | `phase-13/match-check-rename` | 157 | 537k | 15.1M | 101k | **15.8M** | `4a767228` |
| 2026-09-03 08:06 | `phase-13/match-check-rename` | 80 | 219k | 7.9M | 63k | **8.2M** | `df14dea7` |
| 2026-09-04 00:26 | `phase-6.5/upload-stops-blocking` | 348 | 682k | 70.2M | 256k | **71.1M** | `9d5b9e30` |
| 2026-09-04 01:55 | `develop` | 119 | 619k | 15.4M | 88k | **16.1M** | `5b416a79` |
| 2026-09-04 03:38 | `phase-6.5/paste-the-ad` | 87 | 215k | 9.4M | 66k | **9.7M** | `effbf034` |
| 2026-09-04 04:19 | `config/documents-section` | 12 | 104k | 862k | 2k | **969k** | `4de4b279` |
| | | **9077** | **31.6M** | **1384.7M** | **10.1M** | **1426.4M** | 70 sessions |
...
The four sessions that close Phase 6.5 read as a set. `9d5b9e30` is group 6 and went the
way of the sixteen before it — 320 / 62.1M when logged, **348 / 71.1M** when it stopped.
`5b416a79` built group 4, `effbf034` merged it and then added the `Documents` config
section, and `4de4b279` is this one, logging them. Two things in that sequence are worth
more than their token counts.

**The split inside `effbf034` is the interesting one: 8 turns to merge a shipped feature,
73 to add five values to `appsettings.json`.** The mechanical half was nearly free; the
small, careful change was the entire session. That is the opposite of the pattern Phase 2.5
and Phase 5 both found — housekeeping bought expensively at the end of a long session —
and it holds for the same underlying reason read the other way round: the merge replayed
nothing because the work was already decided, and the config change had to decide, for each
constant in the module, whether it was **policy or a column width**. Deliberation is what
costs, not diff size.

And **the four sessions together are 97.9M for what a changelog would render as three
lines.** Group 4 alone is a route, a mutation and a handler that delegates — 16.1M. Nothing
in this ledger has ever made the per-feature number look small, and the reason is in point
3 above rather than in any of these rows.
