...
| ~~**docker-compose**~~ | **Done — 2026-09-01.** `compose.yaml` + `src/Dockerfile` + `web/Dockerfile`: Postgres, the API and Vite in three containers, one command, nothing installed but Docker. It did **not** replace `run.cmd` — that stays the fast inner loop for C#, because the API image is a published build rather than a bind mount. The front end *is* the real dev server with `./web` mounted, on the grounds that a container which took hot reload away would simply go unused. | root `compose.yaml` |
