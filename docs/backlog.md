...
| Candidate | What it is | Cost / size | Likely home | Notes |
|---|---|---|---|---|
| **Rename the ATS check to "Match check", and split off a real ATS check** | The feature called "ATS check" is mostly a CV-vs-one-ad comparison, which is a different product from what the industry calls an ATS check | Medium-large — module name, `ats_results`, the route, both API surfaces and the screen; the split adds a second feature | **P2, and NOT before Phase 13.5** | **Raised by the user, confirmed in code.** `CheckAts.cs:17` lists four stages: resolve, skill gap vs the ad, free-text requirement coverage vs the ad, formatting risks. Only the last — plus contact-detail detection — is a real ATS check. The market splits these cleanly: **ATS-friendliness** (Jobscan, Resume Worded) is CV-only — real text layer or an image, tables/columns/text-boxes/header content that mangle a parse, contact details in the body not the header, standard section headings, parseable file type and dates, no job ad involved. **Match rate** (Jobscan's core, Teal) is the comparison. The file already argues the point without naming it: *"the biggest ATS risk in that document was never keyword coverage — it was that a machine reading the file could not find who the candidate was"*, and then treats that as one stage of four. **Name decided: "Match check"** for the comparison; "ATS check" is freed for the parseability feature. **Sequencing matters** — 13.5 rewrites these endpoints, so a rename now fights it. The Phase 14 skill vocabulary is a prerequisite for the match half either way, and it has landed. See `phases/phase-14-skill-vocabulary.md`. |
