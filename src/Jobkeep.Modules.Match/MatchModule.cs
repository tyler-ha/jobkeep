...
// Module wiring for Match: DI plus its two routes.
//
// ---------------------------------------------------------------------------
...
// coupling is to a shape, not to a lifecycle. Match read and did not write, so
...
// ---------------------------------------------------------------------------
// The old comment named the cost of NOT doing this: extracting Match would stop
...
//     "aggregate in SQL, not in memory". RunMatchCheck.cs argues that in full — the
...
//     because a match result is already a stored judgement about a moment.
//
// What it buys is the thing the phase is for: this module now names one table,
// `match_results`, and it owns it. Lifting it out is a code-move again.
public static class MatchModule
{
    public static IServiceCollection AddMatchModule(this IServiceCollection services)
    {
        ...
        // old comment named. It said: "Match is the only module that is purely a
        // CONSUMER — nothing reads `match_results` except the routes below, so
        // there is nothing for this module to expose. A contract with no caller
        // would be wire schema nobody can safely remove."
        //
        // A caller arrived. 13.3b dropped `match_results.ResumeId` -> `resumes`,
        ...
        // The other half of the same drop: `match_results.ApplicationId` was a
        ...
        // No options class. The two limits RunMatchCheck imposes on the model call are
        ...
        //
        // IChatClient comes from AddModelClient in Program.cs. Match owns the
        // `match_results` table, not the technology (decision 16).
        return services;
    }
}
